        ...
        <div class="row">
            ...
            <div class="col-10 content-section admin-dashboard-title">
                ...
                <ValidationObserver v-slot="{ handleSubmit }">
                    <form v-on:submit.prevent="handleSubmit(editSubscription)">
...
export default {
    ...
    methods: {
        ...
        getSubscription() {
            this.$store.dispatch(GET_SUBSCRIPTION, { id: this.$route.params.subscriptionId })
        },
        getUser() {
            this.$store.dispatch(GET_USER, { id: this.$route.params.userId })
        },
        setValues() {
            this.subscriptionPlan = this.subscription.subscriptionPlan._id
            this.startDate = this.formatDate(this.subscription.startDate)
            this.paymentMethod = this.subscription.payment.paymentMethod._id
            this.transactionRef = this.subscription.payment.transactionRef
            this.currency = this.subscription.payment.currency._id
            this.bank = this.subscription.payment.bank._id
            this.paymentDate = this.formatDate(this.subscription.payment.paymentDate)
            this.isPaid = this.subscription.payment.isPaid
        },
        ...
        editSubscription() {
            ...
            this.$store.dispatch(EDIT_SUBSCRIPTION, payload)        
        }
    },
    mounted() {
